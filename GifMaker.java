...
public class GifMaker {
    ...
    private boolean makeGifWithMediaMetadataRetriever (FFmpegMediaMetadataRetriever retriever, long startMillSeconds, long endMillSeconds, long periodMillSeconds, String outputPath) {
        ...
        try {
            ...
            for (long time = startMillSeconds; time < minDuration; time += periodMillSeconds) {
                bitmaps.add(retriever.getFrameAtTime(time * 1000, FFmpegMediaMetadataRetriever.OPTION_CLOSEST));
            }
            retriever.release();
            return makeGif(bitmaps, outputPath);
        } catch (Exception e) {
